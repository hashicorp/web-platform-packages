    ...
    <div className={classNames(s.cell, isCheckboxCell && s.checkboxCell)}>
      <Dropdown
        renderTrigger={({ onClick }) => (
          <button onClick={onClick} className={s.settingsButton}><FontAwesomeIcon icon={faCog} /></button>
        )}
      >
        <DropdownMenu>
      ...
      {isCheckboxCell ?
        (
          <input
            type="checkbox"
            onChange={(e) => {
              onCellUpdate(index, id as string, e.target.value === 'on' ? true : false);
            }}
          ...
          <TextEditor
            handleChange={(val: string) => {
              onCellUpdate(index, id as string, val);
            }}
            value={value}
            id={id}
            index={index}
