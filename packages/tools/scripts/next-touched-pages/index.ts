...
function printCommentMarkdown(changedPages: string[]): void {
  let comment = '## Changed Pages\n\n'
  if (changedPages.length > 0) {
    changedPages.forEach((p) => {
      comment += `- \`${p}\`\n`
    })
  } else {
    comment += "This PR doesn't appear to affect any pages.\n"
  }

  comment +=
    "\nThis comment was generated by [`next-touched-pages`](https://github.com/hashicorp/web-platform-packages/tree/main/packages/tools/scripts/next-touched-pages), and isn't guaranteed to be accurate."

  comment += '<!-- __NEXTJS_TOUCHED_PAGES -->'
  ...
  try {
    ...
    switch (argv.format) {
      ...
      case 'comment-markdown':
        printCommentMarkdown(sortedChangedPages)
        break
    }
  } catch (err) {
    console.error(err)
