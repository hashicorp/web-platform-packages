...
describe('next-optimized-images/loaders/file-loader', () => {
  ...
  it('uses the correct directory on the server in production', () => {
  ...
  it('uses the assetPrefix config', () => {
    const options1 = getFileLoaderOptions(
      getConfig({ assetPrefix: 'https://cdn.com/' }),
      false
    )
    const options2 = getFileLoaderOptions(
      getConfig({ assetPrefix: 'https://cdn.com' }),
      false
    )
    ...
    expect(options3.publicPath).toEqual(
      'https://cdn.com/_next/static/img-test/'
    )
  })
  ...
  it('overwrites assetPrefix config with imagesPublicPath', () => {
    const options = getFileLoaderOptions(
      getConfig({
        ...
        imagesPublicPath: 'https://another-cdn.com/',
      }),
      false
    )
  ...
  it('allows overwriting the output path', () => {
    const options = getFileLoaderOptions(
      getConfig({ imagesOutputPath: 'another-path/' }),
      true
    )
